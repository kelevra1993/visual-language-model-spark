...
def collate_function(batch: List[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]) -> Tuple[
    ...
    """
    # Extract the individual components from the batched tuples
...
def benchmark_native(data_directory: str, labels_file: str, number_of_runs: int = 10, batch_size: int = 16) -> float:
    ...
    dataloader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=False, num_workers=4, collate_fn=collate_function)
    times = []
