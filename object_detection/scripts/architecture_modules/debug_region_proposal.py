...
def debug_region_proposal() -> None:
    ...
    region_proposal_module = RegionProposal(
    ...
    input_tensor = torch.randn(
        size=(batch_size, input_channels, feature_map_size, feature_map_size), 
        dtype=dtype, 
        device=device
    )
    ...
    target_tensor = torch.randn(
        size=(batch_size, number_of_ground_truth_boxes, 4), 
        dtype=dtype, 
        device=device
    )
    ...
    output = region_proposal_module(input_tensor=input_tensor, target_tensor=target_tensor)

    print_green(output=f"Forward pass completed!")
    print_tensor_status(output[0])
    print_tensor_status(output[0])
